...
export const load = async (event) => {
	async function getIntervention(): Promise<Intervention> {
		const { data: interventions, error: interventionsError } = await event
			.locals.supabase
			.from("interventions")
			.select(
		...
		if (interventionsError) {
			return error(
				500,
				"Error fetching interventions, please try again later.",
			);
		}
	...
	async function getTicketOptions(): Promise<IdAndLabel[]> {
		const { data: tickets, error: ticketsError } = await event.locals
			.supabase
			.from("tickets")
	...
	return {
		...
		updateInterventionForm: await superValidate(
			intervention,
			zod4(createInterventionSchema),
			{

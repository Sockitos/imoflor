...
export const load = async (event) => {
	async function getProperty(): Promise<Property> {
		const { data: property, error: propertyError } = await event.locals
			.supabase
			.from("properties")
		...
		if (propertyError) {
			return error(
				500,
				"Error fetching property, please try again later.",
			);
		}
		return property;
	}
	...
	async function getFractions(): Promise<Fraction[]> {
		const { data: fractions, error: fractionsError } = await event.locals
			.supabase
			.from("properties")
			.select("*, address:addresses(*)")
		...
		if (fractionsError) {
			return error(
				500,
				"Error fetching fractions, please try again later.",
			);
		}
	...
	return {
		...
		updatePropertyForm: await superValidate(
			property,
			zod4(createPropertySchema),
			{
				id: "update-property",
			},
		),
	...
	createFraction: async (event) =>
		handleFormAction(
			...
			async (event, userId, form) => {
				...
				if (addressId == null) {
					const { data: parent, error: parentError } = await event
						.locals.supabase
						.from("properties")
						.select("address_id")
						.eq("id", parentId)
				...
				const { error } = await event.locals.supabase.from("properties")
					.insert({
						...fractionData,

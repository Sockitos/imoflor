<script lang="ts">
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			cell: ({ value, row }) => {
				if (row.isData()) {
					return createRender(PropertyTypeCell, {
						type: value,
					});
				}
				return value ?? '';
		...
		table.display({
			...
			cell: ({ row }) => {
				if (row.isData() && row.original) {
					return createRender(PropertyActions, {
	...
	type PropertyFilters = { type?: PropertyType[] };
	const { filterValues }: { filterValues: Writable<PropertyFilters> } = pluginStates.colFilter;

	const typeOptions = Object.entries(typeMap).map(([value, { label, icon }]) => {
		return { value, label, icon };
	});

	$: counts = properties.reduce(
		(counts, property) => {
		...
		{
			type: {} as Record<PropertyType, number>,
		}
	);

<script lang="ts">
	...
	import { getContract, getContractAccount } from '@/contract/contract.remote';
...
<svelte:boundary>
							...
							<Card.Content>
								<div class="text-2xl font-bold">
									{currencyFormatter.format(
										(getContractAccount(contractId).current ?? [])
					...
					<svelte:boundary>
						...
						{#snippet failed(_, reset)}
							<div class="flex flex-col items-center gap-y-4 px-4 py-6 lg:px-8">
								<p class="text-sm text-destructive">Failed to load contract account.</p>
	...
	{#if contract.type === 'renting'}
		<RentUpdateForm contractId={contract.id} bind:open={openRentUpdateForm} />
	{:else}
		<InstallmentUpdateForm contractId={contract.id} bind:open={openInstallmentUpdateForm} />
	{/if}

	<DueNoteForm contractId={contract.id} bind:open={openDueNoteForm} />

	{#if contract.type === 'renting'}
		<RentPaymentForm contractId={contract.id} bind:open={openPaymentForm} />
	{:else}
		<InstallmentPaymentForm contractId={contract.id} {contract} bind:open={openPaymentForm} />
	{/if}
	...
	{#snippet failed(_, reset)}
		<div class="flex flex-col items-center gap-y-4 px-4 py-6 lg:px-8">
			<p class="text-sm text-destructive">Failed to load contract.</p>
			<Button variant="outline" class="w-fit" onclick={reset}>Retry</Button>
		</div>
	{/snippet}
</svelte:boundary>
